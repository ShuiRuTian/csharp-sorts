...
// Source: upstream/glidesort/src/powersort.rs (entire file).
using System;
...
/// <summary>Powersort's merge-tree node depth math (powersort.rs) — shared by
/// GlideSort and DriftSort, which use identical formulas.</summary>
internal static class Powersort
{
    /// <summary>ceil(2^62 / n) — the [0,1) → [0,2^62) rescaling factor (powersort.rs:48-50).</summary>
    internal static ulong MergeTreeScaleFactor(int n) => ((1UL << 62) + (ulong)(uint)n - 1) / (ulong)(uint)n;

    /// <summary>Desired merge-tree depth of the node between [left, mid) and [mid, right)
    /// (powersort.rs:52-56): CLZ of the XOR of the rescaled (undivided) midpoints.
    /// BitOperations.LeadingZeroCount compiles to a single LZCNT instruction.</summary>
    internal static byte MergeTreeDepth(int left, int mid, int right, ulong scale)
    {
        ulong x = (ulong)(uint)left + (ulong)(uint)mid;
        ulong y = (ulong)(uint)mid + (ulong)(uint)right;
        return (byte)BitOperations.LeadingZeroCount((scale * x) ^ (scale * y));
    }
}
